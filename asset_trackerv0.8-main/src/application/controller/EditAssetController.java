...
//Controller handling EditAsset view
public class EditAssetController {
	...
	//takes selected asset from SearchAsset and loads Asset attributes as default values in the form
	//additionally loads categories and locations drop-down menus
	//resets selection cache
	...
	//show alert if required fields empty
	//updates asset in database using AssetHandler based on user input
	@FXML 
	public void editAsset() {
		if(nameField.getText().isEmpty() || categoryMenu.getSelectionModel().getSelectedItem() == null|| locationMenu.getSelectionModel().getSelectedItem() == null) {
			...
			String response = assetHandler.updateAsset("Name", "'"+name+"'", asset.getName());
			
			if(response == "Error Updating Item") {
				Alert alert = new Alert(Alert.AlertType.ERROR);
				alert.setTitle(response);
				alert.setContentText("Duplicate Asset name"); 
				alert.showAndWait();
			}
			else {
